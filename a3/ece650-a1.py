...
class Street():
    ...
    def get_name(x):
        ...
        if name.replace('"','')[0] == ' ' or name.replace('"','')[-1] == ' ' or '  ' in name:
            print("Error: Check street name", flush=True)
            return None
...
class Graph():
    ...
    def show_graph():
        res = [i for n, i in enumerate(Graph.edges) if i not in Graph.edges[:n]]
        print('V {}'.format(len(Graph.vertices)), flush=True)
        print('E {', end="", flush=True)
        if res:
            for index, item in enumerate(res):
                if index + 1 < len(res):
                    print('<{},{}>,'.format(item[0], item[1]), end="", flush=True)
                else:
                    print('<{},{}>'.format(item[0], item[1]), end="", flush=True)
        print('}', flush=True)       
...
def main():
    while True:
        ...
        if line == "":
            ...
            print('Error: Check format of coordinates', flush=True)
        elif not line.isspace() and line[-2] == " ":
            print('Error: Check trailing spaces', flush=True)
            ...
            if name != None and coord != None:
                if name not in Street.street_names:
                    ...
                    print('Error: Street already exists, try mod command!', flush=True)
            ...
            if name != None and coord != None:
                if name in Street.street_names:
                    Street.street_coords[Street.street_names.index(name)] = Street.get_coords(line)
                else:
                    print('Error: Street does not exist. Try add command!', flush=True)
            ...
            if name in Street.street_names:
                ...
                print('Error: Street does not exist!', flush=True)
            ...
            if len(Street.street_coords)>1:
                            ...
                            if len(a)>1 and len(b)>1:
                                for xi, xitem in enumerate(a):
                                    for yi, yitem in enumerate(b):
                                        intersect_ans = intersect(xitem, a[xi+1], yitem, b[yi+1])
                                        if intersect_ans != None:
                                            if type(intersect_ans[0]) != tuple:
                                                if intersect_ans not in Graph.vertices:
                                                    intersection_list.append(intersect_ans)
                                                    Graph.vertices.append(intersect_ans)
                                                temp_list = [xitem, a[xi+1], yitem, b[yi+1]]
                                                for x in temp_list:
                                                    if x not in Graph.vertices:
                                                        Graph.vertices.append(x)
                                                        Graph.edges.append([Graph.vertices.index(intersect_ans)+1, Graph.vertices.index(x)+1])
                                        if yi+2==len(b):
                                            break
                                    if xi+2==len(a):
                                        break
                ...
                for index, item in enumerate(Graph.edges):
                    ...
                    for item2 in intersection_list:
                        ...
                        x_evaluation = item2[0]>min(x1,x2) and item2[0]<max(x1,x2)
                        y_evaluation = item2[1]>=min(y1,y2) and item2[1]<=max(y1,y2)
                        x_evaluation2 = item2[0]>=min(x1,x2) and item2[0]<=max(x1,x2)
                        y_evaluation2 = item2[1]>min(y1,y2) and item2[1]<max(y1,y2)
                    ...
                    if len(extra_intersections)==1:
                        ...
                        Graph.edges.append([Graph.vertices.index(extra_intersections[0])+1,item[0]])
                        Graph.edges.append([Graph.vertices.index(extra_intersections[0])+1,item[1]])
                    elif len(extra_intersections)>1:
                        pair = [item[0], item[1]]
                        pair.sort()
                        remove_extra_edges.append(item)
                        ans = vertices_sort(extra_intersections)
                        source_first = [pair[0], Graph.vertices.index(extra_intersections[ans[0]])+1]
                        last_dest = [pair[1], Graph.vertices.index(extra_intersections[ans[-1]])+1]
                        ...
                        for index, item in enumerate(ans):
                            src = Graph.vertices.index(extra_intersections[item])
                            des = Graph.vertices.index(extra_intersections[ans[index+1]])
            ...
            print('Error: Invalid command for a1', flush=True)
